...
use Madcoda\Youtube;
wp_cache_delete('fetchTubeResults');
if(!class_exists(fetchTube)) {
	class fetchTube {
		...
		function getClips() {
			...
			#$youtube = new Youtube(array('key' => 'AIzaSyBLWKdh--0x41Je-yZZ6t-lJVdP1EaYolg'));
			#$channel = $youtube->getChannelById($settings['userId'];
...
function get_fetchTube() {
    global $fetchTube;
    // echo '<span style="vertical-align: top;margin-right: 20px;">passion:driving abonnieren</span><div class="g-ytsubscribe" data-channel="vortex3k"></div>';
